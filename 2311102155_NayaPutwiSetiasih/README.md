...
PHP (Hypertext Preprocessor) adalah bahasa pemrograman server-side scripting yang dirancang khusus untuk pengembangan web. PHP pertama kali diciptakan oleh Rasmus Lerdorf pada tahun 1994 dan kini dikelola oleh The PHP Group. Kode PHP dieksekusi di sisi server, kemudian hasilnya dikirim ke browser pengguna dalam bentuk HTML.

Beberapa karakteristik utama PHP antara lain:

- **Server-side**: Kode dijalankan di server, bukan di browser pengguna.
- **Open Source**: Gratis digunakan dan dikembangkan oleh komunitas.
- **Embedded in HTML**: Kode PHP dapat disisipkan langsung di dalam kode HTML menggunakan tag `<?php ... ?>`.
- **Dinamis**: Mampu menghasilkan konten web secara dinamis berdasarkan logika program atau data dari database.
- **Cross-platform**: Dapat berjalan di berbagai sistem operasi seperti Windows, Linux, dan macOS.

Konsep dasar PHP yang digunakan dalam modul ini meliputi:

- **Variabel & Array Asosiatif**: Menyimpan data dalam bentuk pasangan key-value.
- **Function**: Blok kode yang dapat dipanggil berulang kali untuk melakukan tugas tertentu.
- **Operator Aritmatika**: Digunakan untuk melakukan perhitungan matematika (`+`, `-`, `*`, `/`).
- **Operator Perbandingan**: Digunakan untuk membandingkan dua nilai (`>=`, `<=`, `==`, `!=`).
- **Struktur Kontrol if/else**: Menentukan alur program berdasarkan kondisi tertentu.
- **Loop foreach**: Mengiterasi setiap elemen dalam array secara berurutan.
...
Kode PHP tersebut merupakan sebuah sistem sederhana untuk mengelola dan menampilkan penilaian mahasiswa dalam satu halaman web. Di bagian awal, didefinisikan sebuah array asosiatif $mahasiswa yang berisi data beberapa mahasiswa seperti nama, NIM, serta nilai tugas, UTS, dan UAS. Selanjutnya dibuat tiga fungsi utama, yaitu hitungNilaiAkhir() untuk menghitung nilai akhir berdasarkan bobot (tugas 30%, UTS 35%, UAS 35%), tentukanGrade() untuk mengonversi nilai akhir menjadi huruf mutu (A–E), serta tentukanStatus() untuk menentukan apakah mahasiswa tersebut lulus atau tidak berdasarkan nilai akhir (minimal 60 untuk lulus).

Setelah itu, dilakukan proses perulangan menggunakan foreach untuk menghitung nilai akhir setiap mahasiswa, sekaligus menambahkan informasi grade dan status ke dalam array. Pada proses ini juga dihitung total nilai untuk mendapatkan rata-rata kelas, serta dicari nilai tertinggi beserta nama mahasiswa yang mendapatkannya. Kemudian dihitung jumlah mahasiswa dan rata-rata kelas yang ditampilkan di bagian statistik.

Bagian HTML digunakan untuk menampilkan hasil pengolahan data tersebut dalam bentuk tampilan web yang menarik. Terdapat bagian header sebagai judul, lalu bagian “stats” yang menampilkan ringkasan seperti jumlah mahasiswa, rata-rata nilai, nilai tertinggi, dan jumlah mahasiswa yang lulus. Di bawahnya terdapat tabel yang menampilkan data lengkap setiap mahasiswa, mulai dari nama, NIM, nilai tiap komponen, nilai akhir, grade, hingga status kelulusan. Tampilan diperindah menggunakan CSS modern dengan konsep card, badge, dan animasi sederhana agar lebih interaktif dan responsif saat dibuka di berbagai ukuran layar.