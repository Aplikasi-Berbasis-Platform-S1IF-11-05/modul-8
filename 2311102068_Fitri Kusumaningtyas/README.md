...
PHP (Hypertext Preprocessor) adalah bahasa pemrograman server-side yang dirancang khusus untuk pengembangan web dinamis.Berawal dari kebutuhan untuk membuat halaman web yang dapat berinteraksi dengan pengguna dan mengolah data secara otomatis. PHP pertama kali dikembangkan oleh Rasmus Lerdorf pada tahun 1995 sebagai sekumpulan script sederhana untuk melacak kunjungan ke situs pribadinya. Seiring waktu, PHP berkembang menjadi bahasa pemrograman yang lengkap dengan sintaks mirip bahasa C dan Java, mudah dipelajari, serta dapat disisipkan langsung ke dalam HTML. Karena sifatnya yang open source, PHP terus dikembangkan oleh komunitas global dan menjadi salah satu fondasi utama dalam pembuatan aplikasi web modern, termasuk sistem manajemen konten seperti WordPress, Joomla, dan Drupal. 
...
Program diatas merupakan sistem penilaian mahasiswa berbasis PHP yang digunakan untuk mengolah data nilai tugas, UTS, dan UAS menjadi nilai akhir, grade, serta status kelulusan. Pada bagian awal, terdapat array `$mahasiswa` yang berfungsi sebagai tempat penyimpanan data dasar setiap mahasiswa, seperti nama, NIM, dan nilai masing-masing komponen. Setelah itu, terdapat fungsi `hitungNilaiAkhir()` yang digunakan untuk menghitung nilai akhir berdasarkan bobot penilaian, yaitu tugas sebesar 30%, UTS 35%, dan UAS 35%, kemudian hasilnya dibulatkan menjadi dua angka desimal. Selanjutnya, fungsi `tentukanGrade()` berfungsi untuk mengonversi nilai akhir menjadi huruf mutu (A, B, C, D, atau E) berdasarkan rentang nilai tertentu, sedangkan fungsi `tentukanStatus()` digunakan untuk menentukan apakah mahasiswa dinyatakan lulus atau tidak berdasarkan nilai minimal 60.

Dilakukan proses perulangan menggunakan `foreach` untuk setiap data mahasiswa, di mana setiap mahasiswa akan dihitung nilai akhirnya, ditentukan gradenya, serta status kelulusannya. Pada tahap ini juga dilakukan proses akumulasi total nilai untuk menghitung rata-rata kelas, serta pencarian nilai tertinggi beserta nama pemiliknya. Setelah seluruh data diproses, program menghitung jumlah mahasiswa menggunakan `count()` dan menghitung rata-rata kelas dengan membagi total nilai dengan jumlah mahasiswa. Hasil akhir tersebut kemudian ditampilkan dalam bentuk halaman HTML yang sudah didesain menggunakan CSS agar lebih menarik, termasuk tabel data mahasiswa, kartu statistik seperti total mahasiswa, rata-rata kelas, nilai tertinggi, dan jumlah mahasiswa lulus. Selain itu, terdapat fitur visual seperti penanda warna untuk grade dan status, serta highlight khusus untuk mahasiswa dengan nilai tertinggi.
